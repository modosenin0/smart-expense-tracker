...
Write-Host "`n🔑 Assigning Key Vault permissions..." -ForegroundColor Cyan
...
Write-Host "`n🗄️  Generating Database Password..." -ForegroundColor Cyan
...
if ($PostgresServerName) {
    Write-Host "`n🔄 Updating PostgreSQL server password..." -ForegroundColor Cyan
...
Write-Host "`n📊 Creating new Application Insights..." -ForegroundColor Cyan
...
if ($AppServiceName) {
    Write-Host "`n🆔 Enabling Managed Identity for App Service..." -ForegroundColor Cyan
...
Write-Host "`n🔗 Key Vault URLs for secrets:" -ForegroundColor Cyan
...
Write-Host "`n📝 Generating .env template..." -ForegroundColor Cyan
$envContent = @"
...
Write-Host "`n🎉 All secrets are now managed securely in Azure Key Vault!" -ForegroundColor Green